...
FALLBACK_DOCUMENT_PROMPT = """
# ROLE
You are an expert legal-document data extractor.

# TASK
You are given OCR text from an unknown or fallback document in English or Russian.
...
- `description` — the most likely document type or title in Russian
...
class FallbackElseHandler(ConfiguredObjectHandler):
    document_code = "ELSE"
    label = "Unknown / Fallback Document"
    prompt = FALLBACK_DOCUMENT_PROMPT
    examples = ()
    tracked_fields = TRACKED_SIMPLE_DOCUMENT_FIELDS
    empty_error = "No Unknown / Fallback Document fields extracted"
