...
class FastAPISmokeTests(unittest.TestCase):
    ...
    def test_meta_returns_documents_and_models(self):
        ...
        self.assertEqual(payload["documents"][0]["document_code"], "04021")
        self.assertEqual(payload["documents"][0]["label"], "Invoice")
        self.assertIn("schema", payload["documents"][0])
        self.assertEqual(payload["documents"][0]["schema"]["result_type"], "table")
